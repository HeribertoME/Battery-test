  ...
  stages {
    ...
    stage('Analize battery stats') {
      steps {
        ...
        script {
              buildFile = sh (script: '#!/bin/sh python3 ${WORKSPACE}/scripts/readit.py ${WORKSPACE} ${OS_TYPE_PARAM} ${TEST_TYPE_PARAM} ${TEST_TIME_PARAM} ${STRICT_MODE_PARAM} ${APP_TYPE_PARAM}')
        }
      }
    }
